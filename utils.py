...
def to_var(x):
    """
    If there is a GPU that puts X in CUDA 
    """
...
def collate(batch_data):
    """
    Collate is used to determine how the Dataloader generates batch, which is used to sort the padding of sequences of different lengths  
    args:
    ...
    # Processing metadata, merging dict  
...
class Normalizer:
    def __init__(self, dfs=None, variable_length=False):
        ...
        Normalizer
        Args:
            dfs: The list contains each dataframe
            variable_length: Is it variable length data
        """
        self.max_norm = 0
        self.min_norm = 0
        self.std = 0
        ...
        if dfs is not None:
            if variable_length:
                norm_length = min([len(df) for df in dfs])  # Given the variable length data, take the shortest data and make norm
    ...
    def norm_func(self, df):
        df_norm = df.copy()
        (df_norm - self.mean) / np.maximum(np.maximum(1e-4, self.std), 0.1 * (self.max_norm - self.min_norm))
        return df_norm
